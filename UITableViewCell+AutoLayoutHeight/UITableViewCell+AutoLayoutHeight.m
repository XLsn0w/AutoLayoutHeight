                     ...
                     offset:(CGFloat)bottomViewOffset
{
    ...
    if (cellTextWidth == 0) {
        ...
        cellTextWidth = CGRectGetWidth(tableView.frame);
    }
    if (cellTextWidth == 0) {
        return 0;
    }
    cell.frame = CGRectSetWidth(cell.frame, cellTextWidth);
    ...
    CGFloat cellHeight = CGRectGetMaxY(cellBottomView.frame) + bottomViewOffset;
...
+ (CGFloat)layoutCellHeightWithIdentifier:(UITableView *)tableView
                                indexPath:(NSIndexPath *)indexPath
                                    width:(CGFloat)cellTextWidth
                                   offset:(CGFloat)bottomViewOffset {
    NSString *cellIdentifier = [NSString stringWithFormat:@"cellIdentifier_%@", @(indexPath.row)];
    NSLog(@"cellIdentifier===%@", cellIdentifier);
    if (!indexPath || !cellIdentifier) {
        return 0;
    }
    ...
    NSNumber *cacheHeight = tableView.keyCacheDicForCurrentOrientation[cellIdentifier];
    if (cacheHeight != nil) {
        CGFloat cellHeight = 0;
    ...
    CGFloat cellHeight = CGRectGetMaxY(cellBottomView.frame) + bottomViewOffset;
    [tableView.keyCacheDicForCurrentOrientation setValue:@(cellHeight) forKey:cellIdentifier];
    return cellHeight;
}
